...
static inline int ai_should_log(int prio) {
    // El upstream usa __android_log_is_loggable(), que sólo existe desde
    // Android 30 — y ESA era la única razón del minSdk 33 del módulo, no
    // llama.cpp. Aquí se compara contra el nivel mínimo a pelo, que es lo que
    // hace esa función salvo por consultar la propiedad log.tag.* del sistema.
    // Perdemos poder subir el nivel de log con `setprop` en caliente; a cambio
    // el módulo corre desde Android 8 y entra en el parque objetivo de AgentOS.
